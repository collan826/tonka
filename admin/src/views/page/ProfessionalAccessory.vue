    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>{{ pageTitle }}</span>
          <el-button type="primary" @click="handleAdd">+ 新增配件</el-button>
        </div>
      </template>

      <el-table :data="accessoryList" style="width: 100%">
        ...
        <el-table-column prop="title" label="配件名称" />
    ...
    >
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="配件名称">
          <el-input v-model="form.title" placeholder="请输入配件名称" />
        </el-form-item>
...
const fetchList = async () => {
  try {
    const res = await request.get('/api/admin/professional-accessories')
    if (res.code === 200) {
      accessoryList.value = res.data || []
    }
  } catch (error) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除吗？', '提示', {
    ...
    try {
      await request.delete('/api/admin/professional-accessories/' + row.id)
...
const handleSubmit = async () => {
  try {
    if (isEdit.value) {
      await request.put('/api/admin/professional-accessories/' + form.value.id, form.value)
      ElMessage.success('更新成功')
    } else {
      await request.post('/api/admin/professional-accessories', form.value)
