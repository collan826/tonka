...
const fetchList = async () => {
  try {
    const res = await request.get('/api/admin/professional-services')
    if (res.code === 200) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除吗？', '提示', {
    ...
    try {
      await request.delete('/api/admin/professional-services/' + row.id)
...
const handleSubmit = async () => {
  try {
    if (isEdit.value) {
      await request.put('/api/admin/professional-services/' + form.value.id, form.value)
      ElMessage.success('更新成功')
    } else {
      await request.post('/api/admin/professional-services', form.value)
