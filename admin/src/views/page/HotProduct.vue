...
const fetchList = async () => {
  try {
    const res = await request.get('/api/admin/hot-products')
    if (res.code === 200) {
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除吗？', '提示', {
    ...
    try {
      await request.delete('/api/admin/hot-products/' + row.id)
...
const handleSubmit = async () => {
  try {
    if (isEdit.value) {
      await request.put('/api/admin/hot-products/' + form.value.id, form.value)
      ElMessage.success('更新成功')
    } else {
      await request.post('/api/admin/hot-products', form.value)
